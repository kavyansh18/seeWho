...
const Home = () => {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      // Create a set of usernames that you are following
      ...
      // Extract followers' details (href, value, timestamp)
      const followersList = file2Data.map((item: any) => item["string_list_data"][0]);
  
      // Find users who follow you but are not followed by you
      const notFollowedByYouList: User[] = followersList
        .filter((user: any) => !followingList.has(user.value))
        .map((user: any) => ({
          name: user.value,
          href: user.href,
      ...
      setNotFollowedBack(notFollowedByYouList);
      setCount(notFollowedByYouList.length);
    } catch (error) {
    ...
    <div className="bg-gradient-to-r from-rose-100 to-teal-100 w-full min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row justify-start items-start lg:gap-28 gap-12 px-6 lg:px-36">
        <div className="max-w-2xl py-6 mx-8 px-5 scale-125 mt-16 glass ">
          ...
          Check users who are following you, but you don't follow them back
          </h1>
          <div
        ...
        <div className=" px-4 h-[75vh] w-fit mt-3 overflow-y-auto  mb-12 lg:mb-0">
          {notFollowedBack.length > 0 && (
            <div className="mt-6">
              ...
              These people follow you, but you don't follow them back!
              </h2>
