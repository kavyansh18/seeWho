...
const Home = () => {
  ...
  const handleCheck = async () => {
    if (!file) {
      alert("Please upload the pending requests .JSON file.");
      return;
    }
    ...
    try {
      ...
      // Extract the pending requests details (href, value, timestamp)
      const pendingList: User[] = fileData["relationships_follow_requests_sent"]
        .flatMap((request: any) => request["string_list_data"])
        .map((user: any) => ({
          name: user.value,
          href: user.href,
        ...
        <div className=" px-4 h-[75vh] w-fit mt-3 overflow-y-auto  mb-12 lg:mb-0">
          {pendingRequests.length > 0 && (
            <div className="mt-6">
              ...
              These users haven't accepted your follow requests yet
              </h2>
