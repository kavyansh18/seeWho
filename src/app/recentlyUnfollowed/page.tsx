...
const Home = () => {
  ...
  const [unfollowedUsers, setUnfollowedUsers] = useState<User[]>([]);
  ...
  const handleCheck = async () => {
    if (!file) {
      alert("Please upload the recently unfollowed .JSON file.");
      return;
    }
    ...
    try {
      ...
      // Extract the recently unfollowed users' details (href, value, timestamp)
      const unfollowedList: User[] = fileData["relationships_unfollowed_users"]
        .flatMap((userData: any) => userData["string_list_data"])
        .map((user: any) => ({
          name: user.value,
          href: user.href,
      ...
      setUnfollowedUsers(unfollowedList);
      setCount(unfollowedList.length);
    } catch (error) {
      ...
      <div className="flex flex-col lg:flex-row justify-start items-start lg:gap-28 gap-12 px-6 lg:px-36">
        <div className="max-w-2xl py-6 mx-8 px-5 scale-125 mt-16 glass ">
          <h1 className="lg:text-[18px] text-m font-bold ">
            Check recently unfollowed users
          </h1>
          <div
            className="lg:mb-6 mb-4 lg:text-xs text-[9.5px] mt-1 text-red-600 cursor-pointer"
            onClick={openModal}
          >
            How to get this JSON file from Instagram?
          </div>

          <div className="mb-4">
            <label
              className="block mb-2 text-sm font-medium text-gray-900"
              htmlFor="unfollowedFile"
            >
              Upload your recently_unfollowed_profiles.json file
            </label>
            <input
        ...
        <div className=" px-4 h-[75vh] w-fit mt-3 overflow-y-auto  mb-12 lg:mb-0">
          {unfollowedUsers.length > 0 && (
            <div className="mt-6">
              ...
              These users you recently unfollowed
              </h2>
          ...
          >
            ...
            >
              ...
              <ol className="list-decimal">
                <li>Open Instagram, go to settings then <span className="font-semibold text-red-600">Account Center</span>.</li>
                <li>Click on <span className="font-semibold">"Your Information and Permissions"</span></li>
                <li>
                  Click on <span className="font-semibold">"Download your information"</span> then <span className="font-semibold">"Download or transfer information"</span>
                </li>
                ...
                <li>
                  Choose date range according to you and <span className="font-semibold text-red-600">select the format to JSON</span> and high quality, then click create.
                </li>
                <li>
                  Download option will be available in the <span className="font-semibold">Download your information</span> section after a few minutes.
                </li>
                <li><span className="font-semibold text-red-600">Unzip</span> the folder and upload the requested JSON file here.</li>
              </ol>
            </div>
          </div>
        )}
